...
class MFQEv2Dataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        self.mq_root = '/home/data/disk1/xuli/Transcode/DnCNN-PyTorch/MFQEv2_dataset/mfqev2_train_mq.lmdb'
        ...
        self.io_opts_dict['client_keys'] = ['lq','mq', 'gt']
    ...
    def __getitem__(self, index):
        ...
        # get the mq frame (im4.png)
        img_mq_path = key
        img_bytes = self.file_client.get(img_mq_path, 'mq')
        img_mq = _bytes2img(img_bytes)  # (H W 1)
        ...
        img_lp = img_lqs
        img_lp.append(img_mq)
        # randomly crop
        img_gt, img_lp = paired_random_crop(
            img_gt, img_lp, gt_size, img_gt_path
            )

        # flip, rotate
        img_lp.append(img_gt)  # gt joint augmentation with lq
        img_results = augment(
            img_lp, self.opts_dict['use_flip'], self.opts_dict['use_rot']
            )

        # to tensor
        img_results = totensor(img_results)
        img_lqs = torch.stack(img_results[0:-2], dim=0)
        img_mq = img_results[-2]
